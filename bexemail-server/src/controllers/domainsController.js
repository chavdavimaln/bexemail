...
exports.getDomains = async (req, res) => {
  try {
    ...
    if (primaryRows.length > 1) {
      ...
      await pool.query('UPDATE registered_domains SET is_primary = 1 WHERE id = ? AND admin_id = ?', [keepPrimaryId, adminId]);
      rows.forEach(r => {
...
exports.updateDomain = async (req, res) => {
  try {
    ...
    // Check if domain name belongs to another entry for this admin
    const [existing] = await pool.query('SELECT id FROM registered_domains WHERE LOWER(domain_name) = ? AND admin_id = ? AND id != ?', [cleanDomain, adminId, id]);
    if (existing.length > 0) {
      return res.status(400).json({ error: `Domain '${cleanDomain}' is already registered.` });
    }
    ...
    await pool.query(
      'UPDATE registered_domains SET company_name = ?, domain_name = ?, support_email = ?, is_primary = ?, status = ? WHERE id = ? AND admin_id = ?',
      [company_name.trim(), cleanDomain, support_email ? support_email.trim() : null, (is_primary === 1 || is_primary === true) ? 1 : 0, domainStatus, id, adminId]
    );
...
exports.setPrimaryDomain = async (req, res) => {
  try {
    ...
    // Unset is_primary = 0 on ALL domains ONLY for this target admin context, then set target domain to 1
    await pool.query('UPDATE registered_domains SET is_primary = 0 WHERE admin_id = ?', [targetAdminId]);
    await pool.query('UPDATE registered_domains SET is_primary = 1, status = "active" WHERE id = ? AND admin_id = ?', [id, targetAdminId]);
...
exports.toggleDomainStatus = async (req, res) => {
  try {
    ...
    const targetAdminId = currentDomain.admin_id || 1;
    const isCurrentlyActive = currentDomain.status === 'active';
    const newStatusText = isCurrentlyActive ? 'inactive' : 'active';

    // If attempting to deactivate the primary domain, prevent deactivation unless another active domain exists for this admin
    if (currentDomain.is_primary === 1 && isCurrentlyActive) {
      const [otherActive] = await pool.query('SELECT id FROM registered_domains WHERE id != ? AND admin_id = ? AND status = "active" LIMIT 1', [id, targetAdminId]);
      if (otherActive.length > 0) {
        // Promote other active domain to primary for this admin
        await pool.query('UPDATE registered_domains SET is_primary = 1 WHERE id = ? AND admin_id = ?', [otherActive[0].id, targetAdminId]);
        await pool.query('UPDATE registered_domains SET is_primary = 0, status = "inactive" WHERE id = ? AND admin_id = ?', [id, targetAdminId]);
      } else {
        return res.status(400).json({
          error: 'Cannot deactivate primary domain. At least one domain must remain active and primary for your account.'
        });
      }
    } else {
...
exports.deleteDomain = async (req, res) => {
  try {
    ...
    if (totalCount <= 1) {
      return res.status(400).json({ error: 'Cannot delete domain. At least one domain must remain registered for your account.' });
    }

    await pool.query('DELETE FROM registered_domains WHERE id = ?', [id]);

    // If the deleted domain was primary, promote the remaining first domain to primary for this admin
    if (wasPrimary) {
